...
def analyze_one(df, ticker, name=None, market_ok=True):
    ...
    base = {
        ...
        "volumeRatio": sr(volume / avg_volume20 if avg_volume20 else 0),
        "adx": sr(adx),
        "kline": kline,
        "trend": bool(trend),
        "volumeOk": bool(volume_ok),
        "priceVol": bool(price_vol),
        "marketOk": bool(market_ok),
        "score": int(score),
...
def market_analysis(index_data):
    ...
    for label, df in index_data.items():
        if df is None or df.empty or len(df.dropna(subset=["Close"])) < 80:
        ...
        if trend:
            ok_count += 1
        rows[label] = {"status": "站上月線" if close > ma20 else "跌破月線", "close": sr(close), "ma20": sr(ma20), "ma60": sr(ma60), "trend": bool(trend)}
    if ok_count >= 2:
        market_state = "多頭"
        attack = "適合進攻"
        suggestion = "積極"
...
def write_report(rows, meta, market):
    ...
    top = (strict + watch)[:3]
    if strict:
        conclusion = "有符合嚴格條件的美股候選，但仍要等隔日不跌破頸線確認。"
    elif watch:
        conclusion = "今日無完整高勝率標的；以下為接近成形＋可觀察進場3檔。"
    else:
        conclusion = "今日無符合型態學高勝率標的。"
    lines = [
        "# 美股型態學高勝率策略報告",
        "",
        f"更新時間（美東）：{meta.get('updatedAtET')}",
        f"更新時間（台灣）：{meta.get('updatedAtTW')}",
        f"市場狀態：{meta.get('sessionStatus')}",
        "",
        f"## 結論：{conclusion}",
        "",
        "## 美股大盤判斷",
        f"- S&P500：{market['indexes'].get('SPX', {}).get('status')}｜收盤 {market['indexes'].get('SPX', {}).get('close')}｜20MA {market['indexes'].get('SPX', {}).get('ma20')}",
        f"- NASDAQ：{market['indexes'].get('NASDAQ', {}).get('status')}｜收盤 {market['indexes'].get('NASDAQ', {}).get('close')}｜20MA {market['indexes'].get('NASDAQ', {}).get('ma20')}",
        f"- 道瓊：{market['indexes'].get('DJI', {}).get('status')}｜收盤 {market['indexes'].get('DJI', {}).get('close')}｜20MA {market['indexes'].get('DJI', {}).get('ma20')}",
        f"- 當前市場：{market.get('marketState')}",
        f"- 是否適合進攻：{market.get('attack')}",
        f"- 建議：{market.get('suggestion')}",
        "",
        "## 判讀順序",
        "1. 先看結論：有沒有嚴格候選。",
...
def write_outputs(rows, errors, market, total_watchlist):
    ...
    meta = {
        "updatedAtET": now_et().strftime("%Y/%m/%d %H:%M:%S %Z"),
        "updatedAtTW": now_tw().strftime("%Y/%m/%d %H:%M:%S %Z"),
        "sessionStatus": market_session_status(),
        "mode": "us-pattern-high-conviction-scan",
        "source": "Yahoo Finance via yfinance",
        "crossSourceNote": "目前程式化資料源為 Yahoo Finance/yfinance。TradingView、Nasdaq、Google Finance 需正式 API 後才能做嚴格交叉驗證；未驗證不標為多源一致。",
        "totalWatchlist": total_watchlist,
        "totalAnalyzed": len(rows),
        "qualified3Plus": len(qualified),
        "strictCandidates": len(strict),
        "errors": errors[:50],
    }
